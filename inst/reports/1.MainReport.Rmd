...
if (cd4Present) {
  ...
  if (miPresent) {
    ...
    # Removing persons which were MTCT or TRANSFU anywhere (i.e. even in one imputed dataset)
    distr <- data_mi[, .(Count = .N), by = .(Transmission)]
    distr[, Total := sum(Count)]
    distr[, Perc := Count / Total]
    badCategories <- distr[Perc < 0.05, unique(Transmission)]
    if (length(badCategories) > 0) {
      cat(sprintf("<p>Removing persons which were %s anywhere (i.e. even in one imputed dataset)</p>",
                  paste(badCategories, collapse = ", ")))
    }
    badIds <- data_mi[Transmission %chin% badCategories, unique(id)]
    reduced_data_mi <- data_mi[!id %in% badIds]
    reduced_data_mi[, Transmission := droplevels(Transmission)]
    res.list.trans <- mimediqrcd4(vvar = "SqCD4", data = reduced_data_mi, byvar = "Transmission")
...
```{r, eval = cd4Present}
outputIdx <- outputIdx + 1
table2 <- mediqrby(data = data_orig, vvar = "CD4", rowvar = "DateOfDiagnosisYear", colvar = "Gender")
...
```{r, eval = cd4Present}
outputIdx <- outputIdx + 1
table4 <- mediqrby(data_orig, vvar = "CD4", rowvar = "DateOfDiagnosisYear", colvar = "Transmission")
...
numericCols <- setdiff(colnames(tableData), c("DateOfDiagnosisYear"))
tableDataTotals <- tableData[, lapply(.SD, sum), .SDcols = numericCols]
...
tableData <- tableData[, (numericCols) := lapply(.SD, formatNumbers), 
                       .SDcols = numericCols]
setorderv(tableData, "DateOfDiagnosisYear")
...
*Results on CD4 cell count for transmission categories with very low number of cases were either 
not reliable or causing numerical problems thus they are not shown.* 
